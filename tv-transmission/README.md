...
- Add a torrent from a magnet link, `.torrent` URL, or local path (F2)
  — a "Browse..." button next to the field opens tvision's own file
  dialog (filtered to `*.torrent`) and fills the field in with whatever
  gets picked; the field itself still accepts anything typed directly,
  Browse is just a shortcut for the "local file" case
...
**"Add torrent" gained a "Browse..." button** using tvision's own
`TFileDialog` (filtered to `*.torrent`) instead of only accepting a
typed-in magnet link/URL/path. `TFileDialog`'s constructor reads
`TProgram::application->size` to size itself against the current
screen — meaning, like `messageBox()` and a validator's out-of-range
`error()` (see their entries elsewhere in this file), it can only be
constructed with a real `TProgram`/`TApplication` already running.
A standalone test confirmed exactly this: constructing it outside the
real app crashes in that constructor, before anything file-related is
even reached — so neither that constructor nor `getFileName()` could
be exercised in this project's headless tests, only reasoned about by
reading tvision's own source. What *is* tested: the "Add torrent"
dialog's own construction and its field's initial state. The rest —
opening the file dialog and having a click there fill in the field —
is straightforward code (get the chosen name, `setData()` the field)
exercised only by running the real app.

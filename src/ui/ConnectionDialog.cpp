...
// (`new TDialog(...)`) would have done — subclassed only to catch the
// server-name combo's own cmComboBoxSelectionChanged broadcast (see
// TComboBox.h) and load that server's saved connection details into
// the other fields when it fires. Kept local to this file rather than
// given its own header: nothing outside createConnectionDialog() below
...
public:
    ...
    void handleEvent(TEvent& event) override {
        ...
        if (event.what == evBroadcast && event.message.command == cmComboBoxSelectionChanged &&
            fields != nullptr && event.message.infoPtr == fields->serverName) {
            ...
            if (it != current->servers.end()) {
                ...
                // already has a saved profile — a name just typed and
                // added via "[+]", never yet confirmed with OK, has
                // none, and leaving the other fields alone in that case
                // means whatever the user might already be filling in
                // for that new server doesn't get wiped out from under
                // them.
...
TDialog* createConnectionDialog(const AppSettings& current, ConnectionDialogFields& fields) {
    ...
    // than the buttons sitting immediately under Password.
    dlg->insert(new TButton(TRect(20, 17, 30, 19), tr(Str::ButtonOK), cmOK, bfDefault));
